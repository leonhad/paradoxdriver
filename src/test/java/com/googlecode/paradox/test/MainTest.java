...
public class MainTest {
	...
	public void testResultSet2() throws Exception {
		...
		try {
			...
			Assert.assertEquals("1 row: ", "1 - Book 100.0", rs.getLong(1) + " - " + rs.getString(2) + " " + rs.getFloat(3));
			Assert.assertTrue("Second record:", rs.next());
			Assert.assertEquals("2 row: ", "2 - Player 150.0", rs.getLong(1) + " - " + rs.getString(2) + " " + rs.getFloat(3));
			Assert.assertTrue("Third record:", rs.next());
			Assert.assertEquals("2 row: ", "333333333 - Cigar 75.0", rs.getLong(1) + " - " + rs.getString(2) + " " + rs.getFloat(3));
		} finally {
			if (rs != null) {
				rs.close();
			}
